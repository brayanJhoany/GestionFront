<template>
  <div>
    <v-app-bar color="primary" elevate-on-scroll :collapse="false">
      <!-- Logo Institucional -->
    ...
    >
      ...
      <v-btn icon @click="alertaError = false">
        <v-icon color="white">
          fas fa-times-circle
        </v-icon>
      </v-btn>
    ...
    >
      ...
      <strong> {{ textoAlertas }} </strong>
      <v-btn icon @click="alertaExito = false">
        <v-icon class="mr-3" color="white">
          fas fa-times-circle
        </v-icon>
      </v-btn>
...
export default {
  ...
  methods: {
    ...
    obtenerUsuarios() {
      ...
      var c2 = 0;
      var url =
        this.$store.state.rutaDinamica + "api/v1/usuario/listarEncargados";

      axios
        .get(url, this.$store.state.config)
        .then((result) => {
          for (
            let index = 0;
            index < result.data.data.usuarios.length;
            index++
          ) {
        ...
        .catch((error) => {
          if (error.message == "Network Error") {
            ...
            this.textoAlertas =
              "Error al cargar los datos, inténtelo más tarde.";
          } else {
    ...
    contactar() {
      ...
      axios
        .post(url, post, this.$store.state.config)
        .then((result) => {
          this.cargando = false;
          this.dialogContactar = false;
